...
const ExcelImportV6 = (assessmentLevel) => {
  const [jsonData, setJsonData] = useState({});
  const [tableData, setTableData] = useState([]);
  const dispatch = useDispatch();
  const router = useRouter();
  const [fileSelected, setFileSelected] = useState(false); // State to track whether a file has been selected
  ...
  useEffect(() => {
    ...
    Object.keys(jsonData).forEach((sheetName) => {
      ...
      if (sheetName === "cover") {
        ...
        console.log("inside data protection", sheetData);
        const Data = convertDataprotection(sheetData);

        if (Data) {
          let parentKey = "", parentKeyQuestion = "";
          let subKey = "", subKeyQuestion = "";
          ...
          Data.forEach((item) => {
            ...
            if (single) {
              ...
              parentKeyQuestion = item["Control question"];
            }

            if (double) {
              subKey = item["ISA New"].trim();
              subKeyQuestion = item["Control question"];
            }

            if (triple) {
              const idParts = item["ISA New"].split(".");
              const tripleParentKey = idParts[0];
              const tripleSubKey = idParts.slice(0, 2).join(".");

              if (tripleParentKey === parentKey) {
                item["Parent ISA New"] = parentKey;
                item["Parent Control question"] = parentKeyQuestion;
              } else {
                item["Parent ISA New"] = "";
                item["Parent Control question"] = "";
              }

              if (tripleSubKey === subKey) {
                item["Parent ISA New"] = subKey;
                item["Parent Control question"] = subKeyQuestion;
              } else {
                item["Parent ISA New"] = "";
                item["Parent Control question"] = "";
              }
        ...
        // Handle prototype protection and information security
      ...
      Object.keys(item).forEach((key) => {
        ...
        switch (true) {
          case modifiedKey.toLowerCase().includes("(must)"):
            modifiedItem["Requirements"] = [item[key]];
            break;
  ...
  const groupByDatas = (data) => {
    ...
    data.forEach((item) => {
      ...
      Object.keys(item).forEach((key) => {
        ...
        switch (true) {
          case modifiedKey.toLowerCase().includes("(must)"):
            modifiedItem["Must Requirements"] = validCheck
              ? ""
              : [item[key]];
            break;
          case modifiedKey.toLowerCase().includes("referenz dokumentation") ||
      ...
      workbook.SheetNames.forEach((sheetName) => {
        ...
        if (
          ...
          if (compareTwoStrings(trimmedSheetName, "information security") > 0.6) {
            ...
            const informationSecurityHeaderIndexNumber = 1;
            const isaNewColumnIndex = jsonData[informationSecurityHeaderIndexNumber].indexOf('Control number');

            if (isaNewColumnIndex !== -1) {
              jsonData[informationSecurityHeaderIndexNumber][isaNewColumnIndex] = "ISA New";
            }

            const startColumnIndex = XLSX.utils.decode_col("C");
          ...
          } else if (compareTwoStrings(trimmedSheetName, "data protection") > 0.6) {
            let jsonData = XLSX.utils.sheet_to_json(sheet, {
              header: 1,
              range: XLSX.utils.decode_range(sheet["!ref"]),
              defval: "",
            ...
            const dataProtectionHeaderIndexNumber = 3;
            const isaNewColumnIndex = jsonData[dataProtectionHeaderIndexNumber].indexOf('Control number');

            if (isaNewColumnIndex !== -1) {
              jsonData[dataProtectionHeaderIndexNumber][isaNewColumnIndex] = "ISA New";
            }
            ...
            jsonData.forEach((row) => {
              for (let colIndex = startColumnIndex; colIndex <= endColumnIndex; colIndex++) {
                if (row[colIndex] !== undefined) {
                  row[colIndex] = String(row[colIndex]);
                }
              }
            });

            const dataprosheetData = jsonData
              .slice(dataProtectionHeaderIndexNumber, rowWithCondition + 1)
              .map((row) =>
                row.filter(
                  (cell, columnIndex) =>
                    columnIndex >= startColumnIndex &&
          ...
          } else if (compareTwoStrings(trimmedSheetName, "prototype protection") > 0.6) {
            let jsonData = XLSX.utils.sheet_to_json(sheet, {
              header: 1,
              range: XLSX.utils.decode_range(sheet["!ref"]),
              defval: "",
            ...
            const prototypeProtectionHeaderIndexNumber = 1;
            const isaNewColumnIndex = jsonData[prototypeProtectionHeaderIndexNumber].indexOf('Control number');

            if (isaNewColumnIndex !== -1) {
              jsonData[prototypeProtectionHeaderIndexNumber][isaNewColumnIndex] = "ISA New";
            }
  ...
  const convertDataprotection = (sheetdata) => {
  ...
  return (
    <div>
      <input
        ...
        disabled={!assessmentlevel} // Disable file input if assessment level is not selected or no file is selected
      />

      {tableData.length > 0 && (
        <Link
          href={`/importtisax/TisaxsTabs?assessment_level=${assessmentlevel}&vda_version=6.0.3`}
        >
          <button
